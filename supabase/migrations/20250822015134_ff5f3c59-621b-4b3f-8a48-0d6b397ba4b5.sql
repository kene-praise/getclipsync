-- Drop ALL existing RLS policies for temporary clips and files
...
DROP POLICY IF EXISTS "Public can create temporary clip files" ON public.temporary_clip_files;
...
-- Only allow inserts for creating new clips/files
CREATE POLICY "Allow creating temporary clips" 
ON public.temporary_clips 
FOR INSERT 
WITH CHECK (true);

CREATE POLICY "Allow creating temporary clip files" 
ON public.temporary_clip_files 
FOR INSERT 
WITH CHECK (true);

-- Deny all direct SELECT access - must use the secure function
CREATE POLICY "Deny direct SELECT on temporary clips" 
ON public.temporary_clips 
FOR SELECT 
USING (false);

CREATE POLICY "Deny direct SELECT on temporary clip files" 
ON public.temporary_clip_files 
FOR SELECT 
USING (false);